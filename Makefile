...
docker-setup: ## Create necessary directories for Docker
	mkdir -p auths

docker-build: ## Build Docker image
	docker build -t qwen-go-proxy .

docker-run: docker-setup ## Run Docker container
	docker run -p 8080:8080 --env-file .env -v $(PWD)/auths:/app/.qwen qwen-go-proxy

docker-compose-up: docker-setup ## Start with docker-compose
	docker compose up -d
