...
func LoadConfig() (*entities.Config, error) {
	...
	config := &entities.Config{
		...
		StreamingMaxErrors:           getEnvIntWithDefault("STREAMING_MAX_ERRORS", 10),
		StreamingBufferSize:          getEnvIntWithDefault("STREAMING_BUFFER_SIZE", 4096),
		StreamingTimeoutSeconds:      getEnvIntWithDefault("STREAMING_TIMEOUT_SECONDS", 900),
		StreamingWindowSize:          getEnvIntWithDefault("STREAMING_WINDOW_SIZE", 5),
		StreamingSimilarityThreshold: getEnvFloatWithDefault("STREAMING_SIMILARITY_THRESHOLD", 0.8),
		StreamingTimeWindow:          getEnvDurationWithDefault("STREAMING_TIME_WINDOW", 2*time.Second),
		StreamingMinConfidence:       getEnvFloatWithDefault("STREAMING_MIN_CONFIDENCE", 0.7),
