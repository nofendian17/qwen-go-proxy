...
func TestLoadConfig_DefaultValues(t *testing.T) {
	...
	assert.Equal(t, 10, config.StreamingMaxErrors)
	assert.Equal(t, 4096, config.StreamingBufferSize)
	assert.Equal(t, 900, config.StreamingTimeoutSeconds)
	assert.Equal(t, 5, config.StreamingWindowSize)
	assert.Equal(t, 0.8, config.StreamingSimilarityThreshold)
	assert.Equal(t, 2*time.Second, config.StreamingTimeWindow)
	assert.Equal(t, 0.7, config.StreamingMinConfidence)
...
func TestLoadConfig_WithEnvVars(t *testing.T) {
	...
	assert.Equal(t, 20, config.StreamingMaxErrors)
	assert.Equal(t, 8192, config.StreamingBufferSize)
	assert.Equal(t, 1800, config.StreamingTimeoutSeconds)
	assert.Equal(t, 10, config.StreamingWindowSize)
	assert.Equal(t, 0.9, config.StreamingSimilarityThreshold)
	assert.Equal(t, 5*time.Second, config.StreamingTimeWindow)
	assert.Equal(t, 0.8, config.StreamingMinConfidence)
...
func clearEnvVars() {
	envVars := []string{
		...
		"STREAMING_MAX_ERRORS", "STREAMING_BUFFER_SIZE", "STREAMING_TIMEOUT_SECONDS",
